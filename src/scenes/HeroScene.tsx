...
// Commissioned separately (an AI product-film prompt, not generated here) —
// a single laptop on a pure-black studio background, isolated to the RIGHT
// of frame with large negative space on the LEFT, an extremely slow dolly-in,
// shot specifically to be scrubbed by scroll position rather than played
// back in real time. Neither file exists in the repo yet; the user supplies
// both once ready. Until then this renders correctly — see the
// missing-asset handling below.
const VIDEO_SRC = "/kov/home/laptop-hero.mp4";
const POSTER_SRC = "/kov/home/laptop-hero-poster.jpg";
...
// The back half of the dolly-in should read as clean, uncluttered product
// photography (the brief's own words: "the laptop remains the visual anchor
// throughout") — text dissolves out over this scroll-progress window rather
// than sitting glued on screen for the whole scrub.
...
// Darkest over the text column (left), fading toward transparent where the
// laptop sits (right) — legibility without washing out the footage itself.
...
export function HeroScene() {
  ...
  useEffect(() => {
    ...
    function applyFrame() {
      rafId = null;
      // Guards the file not existing yet (true today): loadedmetadata never
      // fires, duration stays 0, this just no-ops forever instead of
      // writing NaN/undefined into currentTime.
