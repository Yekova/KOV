...
// Vendored from reactbits.dev (DepthCarousel, MIT-licensed) — kept close to
// upstream (the layout/drag/wheel/tween math is unchanged). One structural
// fix: upstream assigns `cfgRef.current`/`onChangeRef.current` directly in
// the component body during render, which is a ref mutation during render
// — moved into a ref-sync `useEffect` with no dependency array instead
// (same fix already applied to CursorGrid.tsx and the pattern
// SpecularButtonEffect.tsx already established in this codebase).
import {
  useCallback,
  useEffect,
  useMemo,
...
type DepthCarouselItemInput = string | DepthCarouselItem;
...
const normalizeItem = (it: DepthCarouselItemInput): DepthCarouselItem => (typeof it === "string" ? { image: it, alt: "" } : it);
...
export default function DepthCarousel({
  ...
  // during render — see file-level comment.
  useEffect(() => {
  ...
  return (
    <div
      ...
      aria-label="Photos du studio KOV"
      tabIndex={0}
