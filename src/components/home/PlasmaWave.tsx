...
// Ported from React Bits (reactbits.dev/backgrounds/plasma-wave) — kept
// close to the original source rather than rewritten against KOV's own
// render stack, same reasoning as GlassSurface.tsx: a self-contained
// vendored shader effect isn't worth re-deriving by hand. `ogl` was
// already a dependency (ColorBends' sibling React Bits ports use it).
...
export default function PlasmaWave(props: PlasmaWaveProps) {
  ...
  useEffect(() => {
    ...
    // Intentionally empty — this effect creates the GL context/program
    // once; the rAF loop above reads the latest values every frame via
    // propsRef.current instead of closing over these props, so a prop
    // change never tears down and rebuilds the WebGL context.
