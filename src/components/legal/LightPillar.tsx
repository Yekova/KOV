...
// Vendored from reactbits.dev (DavidHDev/react-bits, Backgrounds/LightPillar,
// MIT-licensed) rather than hand-rolled — this is a raymarched WebGL shader
// effect, not an interaction/animation pattern reasonable to reimplement from
// scratch. Kept close to the upstream source (shader and render-loop logic
// unchanged); the only deliberate deviations are the TypeScript types and the
...
export default function LightPillar({
  ...
  // Same reduced-motion convention as every other WebGL/GSAP effect in this
  // codebase (see LiquidEther.tsx, SmoothScroll.tsx) — a continuously
  // rendering raymarched shader is exactly the kind of motion that gate
  ...
  useEffect(() => {
    ...
    try {
      ...
      // The lazy-init probe above already gates this effect on
      // `webGLSupported` — reaching here despite that would mean context
      // creation succeeded once but failed moments later (driver reset,
      // context loss). Bailing out without a setState call keeps this
      // effect from tripping the "no setState in an effect body" rule for
      // a case that isn't actually a synchronous derived-state update;
      // it's just an abort of this mount attempt.
      console.error("LightPillar: WebGLRenderer construction failed after the capability probe succeeded", error);
      return;
    }

    renderer.setSize(width, height);
    renderer.setPixelRatio(effectiveQuality === "high" ? Math.min(window.devicePixelRatio, 2) : settings.pixelRatio);
  ...
  }, [webGLSupported, quality, reducedMotion]);
