...
// Vendored from reactbits.dev (MagicRings, MIT-licensed) — kept intentionally
// close to the upstream source (shader and animation-loop logic unchanged),
// same rationale as LiquidEther.tsx: subtle edits to this kind of numerical/
// WebGL code risk silently breaking the effect in ways hard to notice by
// inspection. The only real additions are TypeScript types and the
// reduced-motion gate (see below) — every other WebGL effect in this
// codebase checks prefers-reduced-motion; the upstream component didn't.
...
export default function MagicRings({
  ...
  // ShapeBlur.tsx/LiquidEther.tsx.
  const [reducedMotion] = useState(
    () => typeof window !== "undefined" && window.matchMedia("(prefers-reduced-motion: reduce)").matches
  );

  // Keeps the long-lived rAF loop's closure reading fresh props without
  // restarting the WebGL setup effect below — refs can't be written during
  // render, so this runs post-render every time instead (no deps array).
  useEffect(() => {
    propsRef.current = {
  ...
  useEffect(() => {
    ...
    const animate = (t: number) => {
      ...
      const p = propsRef.current as {
        speed: number; attenuation: number; color: string; colorTwo: string;
        lineThickness: number; baseRadius: number; radiusStep: number; scaleRate: number;
        ringCount: number; opacity: number; noiseAmount: number; rotation: number;
        ringGap: number; fadeIn: number; fadeOut: number; followMouse: boolean;
        mouseInfluence: number; hoverScale: number; parallax: number; clickBurst: boolean;
        alphaMode: "luminance" | "coverage";
      };
