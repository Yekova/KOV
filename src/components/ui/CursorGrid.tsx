...
// Vendored from reactbits.dev (CursorGrid, MIT-licensed) — kept close to
// upstream (same cell/energy/fade math), with two deliberate deviations:
//
// 1. Pointer listeners attach to `window`, not this component's own
//    container, and the container is `pointer-events: none` (see
//    CursorGrid.css). Upstream's demo usage is a full-bleed standalone
//    box with nothing on top of it; here it's a background layer behind
//    real, clickable content (search box, filter pills, accordion rows),
//    so it can never be a hit target itself. This is the exact pattern
//    LiquidEther.tsx already uses for the same reason (its Mouse class
//    listens on the container's ownerDocument's defaultView, i.e.
//    `window`) — `getBoundingClientRect()`-based coordinate translation
//    works the same regardless of which element the listener is
//    attached to, so this doesn't touch the simulation math at all.
// 2. A reduced-motion gate was added (missing upstream) — matches every
//    other cursor-tracking canvas effect in this codebase.
...
export default function CursorGrid({
  ...
  // Kept in sync every render (no deps array) rather than mutated inline
  // during render — the draw loop reads props live off this ref via
  // closures that outlive any single render, so it needs updating on
  // every commit, but a ref mutation has to happen in an effect, not the
  // render body itself.
  useEffect(() => {
    propsRef.current = {
      cellSize,
  ...
  useEffect(() => {
    ...
    const toLocal = (event: MouseEvent | PointerEvent): [number, number] => {
    ...
    // Listen on window, not `container` — see the file-level comment for
    // why (this element is pointer-events: none so real content on top
    // stays clickable). `toLocal` translates via the canvas's own
    // getBoundingClientRect(), so the coordinate math is unaffected by
    // which element the listener is actually attached to.
