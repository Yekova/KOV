...
| `/kov/character/login-frames/frame-000.jpg` … `frame-059.jpg` | Séquence de 60 images extraites d'un clip Kling 16:9 (rotation face → profil droit, personnage statique) — utilisée par `/login`, indexée en direct par la position horizontale de la souris |
| `/kov/character/contact-frames/frame-000.jpg` … `frame-059.jpg` | Séquence de 60 images extraites d'un second clip Kling 16:9 distinct — personnage vu de dos qui s'éloigne dans le couloir en béton vers la porte éclairée, un mur de lumière blanche au fond et une ligne rouge sur le mur latéral — utilisée par `/contact`, même mécanisme (souris → frame), animation structurellement différente (marche/éloignement plutôt que rotation sur place) |

*(2026-08-20 : remplace l'ancien placeholder procédural Three.js de `/login` (voir `src/components/ui/MouseFrameBackdrop.tsx`, composant partagé par les deux pages) ainsi que l'ancien panneau `/contact` à glisser-déposer encadré (`ScrubVideo`/`ContactCharacterPanel`, supprimés). Le premier essai login en 9:16 laissait trop de vide à l'écran une fois en fond de page ; regénéré en 16:9 avec le personnage centré et dominant le cadre. Aucun modèle Kling n'expose de contrôle d'orbite caméra précis ; les clips sont un mouvement décrit par prompt, pas un tourniquet 3D exact — la rotation login va d'un cadrage face-caméra à un profil droit complet, pas symétrique gauche/droite. Fait notable : contrairement à la rotation statique de login, le personnage de contact change de taille tout du long (il s'éloigne) — la hauteur de la tête à l'écran reste néanmoins étonnamment stable d'un bout à l'autre (loi de la perspective pour un sujet qui recule face à une caméra fixe et de niveau), ce qui permet d'appliquer un unique décalage de recomposition à toutes les frames sans réajustement par frame.
